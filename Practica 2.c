...
int busqueda_indexada(struct compras *ventas,int n, int m, int ganador){
    ...
    return binary_search(ventas,ini,fin,ganador);
}

int binary_search(struct compras *ventas, int ini, int fin, int ganador){
	int medio;
	if(ini>=0){
		medio=ini+(fin-ini)/2;	
		if(ventas[medio].numero==ganador)
			return comprobar_boleto(ventas,medio);
		if(ventas[medio].numero>ganador)
			return binary_search(ventas,ini,medio-1,ganador);
		if(ventas[medio].numero<ganador)
			return binary_search(ventas,medio+1,fin,ganador); 
	}
	return -1;
}

int comprobar_boleto(struct compras *ventas,int pos){
	int resultado;
	resultado = strcmp(ventas[pos].nombre, "Sin asignar");
	if(resultado!=0)
...
int main(){
        ...
        do{
                ...
                if(premio==0)
                	printf("el boleto no fue asignado\n");
        }while(premio>=0);
}
